// 订阅话题方，订阅要求的话题，并以/car/cones话题发布出去
...
void doMsg(const plumbing_pub_sub::ConeArray::ConstPtr &msg)
{
    ...
    for(size_t i = 0; i < msg->cones.size(); ++i)
    {
        ...
        double w = msg->cones[i].position.z;
        // ROS_INFO("数据u,v为：%.2lf,%.2lf,%.2lf",u,v,w);
        ...
        double car_z = TZ + base_z;

        ROS_INFO("锥桶 [%zu]: camera_pos=(%.2lfm, %.2lfm, %.2lfm), color='%s', car_pos=(%.2lfm, %.2lfm, %.2lfm)", 
                  i + 1,
                  x_m,
                  y_m,
                  z_m,
