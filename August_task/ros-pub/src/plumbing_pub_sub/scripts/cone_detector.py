...
# -*- coding: utf-8 -*-
"""
cone_detector.py — YOLOv8 锥桶检测 ROS 节点
===========================================
功能：订阅图像话题 → YOLOv8 推理 → 发布 ConeArray 到 /test/camera_cones

这是感知链路的"头"节点，承接 YOLO 检测结果并发布给下游坐标转换节点。

订阅话题：
    <image_topic> (sensor_msgs/Image 或 sensor_msgs/CompressedImage)

发布话题：
    /test/camera_cones (plumbing_pub_sub/ConeArray)
        每个 ConeInfo:
            position.x = 检测框中心 x（像素）
            position.y = 检测框中心 y（像素）
            position.z = 0（暂不使用）
            color      = "red_cone" / "blue_cone" / "yellow_cone"

参数（rosrun 传参或 launch 文件配置）：
    ~image_topic    : 订阅的图像话题名（默认: /pylon_camera_node/image_raw）
    ~model_path     : YOLO 模型路径（默认: yolov8n.pt）
    ~conf_threshold : 置信度阈值（默认: 0.5）
    ~publish_2d     : 是否同时发布 2D 检测结果到 /test/camera_cones_2d（默认: false）
                    话题类型: plumbing_pub_sub/ConeDetection2DArray

用法：
    # 方式1：rosrun
    rosrun plumbing_pub_sub cone_detector.py \
        _image_topic:=/camera/image_raw \
        _model_path:=/home/xiaoyang/桌面/August_task/results/best.pt

    # 方式2：launch 文件
    <node pkg="plumbing_pub_sub" type="cone_detector.py" name="cone_detector" output="screen">
        <param name="image_topic" value="/pylon_camera_node/image_raw"/>
        <param name="model_path" value="/home/xiaoyang/桌面/August_task/results/best.pt"/>
...
# ---- 环境兼容 ----
# 如果由系统 Python 启动（非 venv），补充 venv 路径以找到 ultralytics 等包
_VENV_SITE = os.path.expanduser('~/桌面/August_task/.venv/lib/python3.8/site-packages')
...
from plumbing_pub_sub.msg import ConeInfo, ConeArray, ConeDetection2D, ConeDetection2DArray
...
class ConeDetector:
    ...
    def _subscribe_image_topic(self, topic_name):
        ...
        # 轮询等待话题在 ROS master 中注册（最多等 15 秒）
        msg_type_str = None
        ...
        if results and len(results) > 0:
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    # ---- 构造 ConeInfo（对齐组间接口: +header +confidence） ----
                    cone_info = ConeInfo()
