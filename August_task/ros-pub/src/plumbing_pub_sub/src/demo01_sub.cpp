/**
 * demo01_sub.cpp — 像素坐标 → 车体坐标
 * ============================================================
 * 坐标转换流程（与 YOLOv7/yolov7/detect_ros.py 一致）:
 *   1. 深度从 bbox 高度估算: depth = fy * cone_h / pixel_height
 *   2. back-project: Xc=(u-cx)*depth/fx, Yc=(cy-v)*depth/fy, Zc=depth
 *      (相机帧: X右 Y上 Z前)
 *   3. 相机→车体: R_cam2veh * [Xc,Yc,Zc] + T
 *      R = [[0,0,1],[-1,0,0],[0,1,0]], T = [0.2,0.1,0.45]
 */
...
int main(int argc, char *argv[])
{
    ...
    g_cone_pub = nh_global.advertise<plumbing_pub_sub::ConeArray>("/yolov7/yolov7/all_cones", 10);
    ros::Subscriber sub = nh_global.subscribe<plumbing_pub_sub::ConeArray>(
...
void doMsg(const plumbing_pub_sub::ConeArray::ConstPtr &msg)
{
    ...
    plumbing_pub_sub::ConeArray car_cones;
    ...
    for (size_t i = 0; i < msg->cones.size(); ++i)
    {
        ...
        // 1. back-project → 相机坐标系 (X右 Y上 Z前, 对齐学长代码)
        ...
        plumbing_pub_sub::ConeInfo cone;
